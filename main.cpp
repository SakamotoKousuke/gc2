	...
	while (std::getline(file, line)) {
		...
		if (identifier == "v") {
			...
			for (int32_t faceVertex = 0; faceVertex < 3; ++faceVertex) {
				...
				//tringle[faceVertex] = { position,texcoord,normal };
			...
			// materialTemplateLibrary ファイルの名前を取得する
...
std::wstring ConvertString(const std::string& str) {
	...
	MultiByteToWideChar(CP_UTF8, 0, reinterpret_cast<const char*>
		(&str[0]), static_cast<int>(str.size()), &result[0], sizeNeeded);
	return result;
}
	...
	IDxcIncludeHandler* includeHandler)
{
	...
	LPCWSTR arguments[] = {
		...
		L"-E", L"main", // エントリーポイントの指定。基本的にmain以外にはしっ 
...
ID3D12Resource* CreateTextuerResource(ID3D12Device* device, const DirectX::TexMetadata& metadata)
{
	...
	// TextureのFormat
	resourceDesc.SampleDesc.Count = 1; // サンプリングカウント。1固定。
	resourceDesc.Dimension = D3D12_RESOURCE_DIMENSION(metadata.dimension); // Textureの次元数。普段使っ
...
ID3D12Resource* CreateBufferResource(ID3D12Device* device, size_t sizeInBytes)
{
	...
	HRESULT hr = device->CreateCommittedResource(&uploadHeapProperties, D3D12_HEAP_FLAG_NONE, &vertexResourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ,
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	const char* featureLevelStrings[] = { "12.2", "12.1", "12.8" };
	...
	for (size_t i = 0; i < _countof(featureLevels); ++i) {
		...
		if (SUCCEEDED(hr)) {
			...
			Log(std::format("D3D12CreateDevice failed for FeatureLevel {}. HRESULT: 0x{:08X}\n", featureLevelStrings[i], hr));
		}
	}
	...
	//現時点でincludeはしてないが、includeに対応するための設定を行っておく
	...
	// Root Parameter作成。複数設定できるので配列。今回は結果1つだけなので長さ1の配列 
	...
	rootParameters[2].DescriptorTable.NumDescriptorRanges = _countof(descriptorRange); // Table
	...
	//// 左下 
	//vertexData[3].position = { -0.5, -0.5f, 0.5f, 1.0f };
	...
	vertexResource->Map(0, nullptr, reinterpret_cast<void**>(&vertexData));
	// 書き込むためのアドレスを取得
	std::memcpy(vertexData, modelData.vertices.data(), sizeof(VertexData) * modelData.vertices.size()); //頂点データ
...
// こういうもんである
	...
	const DirectX::TexMetadata& metadata = mipImages.GetMetadata();
	ID3D12Resource* textureResource = CreateTextuerResource(device, metadata);
	...
	*transformationMatrixDataSprite = MakeIdentity4x4();
	...
	Microsoft::WRL::ComPtr<ID3D12Resource> instancingResource = CreateBufferResourece(device, sizeofT(ransformationMatrix) * kNumInstance);
	//書き込むためのアドレスを取得
	TransformationMatrix* instancingData = nullptr;
	instancingResource->Map(0, nullptr, reinterpret_cast<void**>(&instancingData));
	//単位行列を書き込んでおく
	for (unit32_t index = 0; index < kNumInstance; ++index) {
	...
	while (msg.message != WM_QUIT) {
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			*transformationMatrixDataSprite = worldViewProjectionMatrixSprite;
			...
			// TransitionBarrierを張る commandList->ResourceBarrier(1, &barrier);
